		    ...
		    IPointer<Integer> pMax)
		{
		    ...
		    if( aiPropertyTypeInfo.aiPTI_Float == prop.mType || aiPropertyTypeInfo.aiPTI_Buffer == prop.mType)  {
		        ...
		        for (int a = 0; a < iWrite;++a)    {
		            pOut[a] = new ai_real(bb.getFloat());
		        }
		        if (pMax.get()!=null) {
		            pMax.set(iWrite);
		        }
		    ...
		    // data is given in doubles, convert to float
		        ...
		        for (int a = 0; a < iWrite;++a)    {
		        	 pOut[a] = new ai_real((float)bb.getDouble());
		        }
		        if (pMax.get()!=null) {
		        	pMax.set(iWrite);
		        }
		    ...
		    // data is given in ints, convert to float
		        ...
		        for (int a = 0; a < iWrite;++a)    {
		        	 pOut[a] = new ai_real(bb.getInt()+0.0F);
		        }
		        if (pMax.get()!=null) {
		        	pMax.set(iWrite);
		        }
...
//		        char cur = prop.mData + 4;
		        ...
		        for ( int a = 0; ;++a) {
		        	IPointer<Real> temp = Pointer.valueOf(new Real(0.0F)); System.out.println(StringUtil.getCharactersAsString(pointer, 17));
		            pointer = fast_atof.fast_atoreal_move(pointer,temp); 
		            pOut[a]=new ai_real(temp.get().getValue());
		            if ( a==iWrite-1 ) {
		                break;
		            }
