...
import com.trivialis.java.jassimp.port.include.assimp.postprocess;
...
public class ConvertToLHProcess {
	...
	public static class MakeLeftHandedProcess {
		...
		public boolean IsActive(int pFlags) {
			return 0 != (pFlags & postprocess.aiProcess_MakeLeftHanded);
		}
		...
		public void ProcessNode(aiNode pNode, aiMatrix4x4 pParentGlobalRotation)
		{
		    ...
		    pNode.mTransformation.c1 = pNode.mTransformation.c1* -1.0F;
		    pNode.mTransformation.c2 = pNode.mTransformation.c2* -1.0F;
		    pNode.mTransformation.c3 = pNode.mTransformation.c3* -1.0F;
		    pNode.mTransformation.c4 = pNode.mTransformation.c4* -1.0F;

		    // now invert the Z axis again to keep the matrix determinant positive.
		    // The local meshes will be inverted accordingly so that the result should look just fine again.
		    pNode.mTransformation.a3 = pNode.mTransformation.a3* -1.0F;
		    pNode.mTransformation.b3 = pNode.mTransformation.b3* -1.0F;
		    pNode.mTransformation.c3 = pNode.mTransformation.c3* -1.0F;
		    pNode.mTransformation.d3 = pNode.mTransformation.d3* -1.0F; // useless, but anyways...
		...
		public void ProcessMesh(aiMesh pMesh)
		{
		    ...
		    for( int a = 0; a < pMesh.mNumBones; ++a)
		    {
		        ...
		        bone.mOffsetMatrix.a3 = bone.mOffsetMatrix.a3* -1.0F;
		        bone.mOffsetMatrix.b3 = bone.mOffsetMatrix.b3* -1.0F;
		        bone.mOffsetMatrix.d3 = bone.mOffsetMatrix.d3* -1.0F;
		        bone.mOffsetMatrix.c1 = bone.mOffsetMatrix.c1* -1.0F;
		        bone.mOffsetMatrix.c2 = bone.mOffsetMatrix.c2* -1.0F;
		        bone.mOffsetMatrix.c4 = bone.mOffsetMatrix.c4* -1.0F;
		    }

		    // mirror bitangents as well as they're derived from the texture coords
		    if( pMesh.HasTangentsAndBitangents()) //We never get here!?
		    {
		        for( int a = 0; a < pMesh.mNumVertices; a++){System.out.println(pMesh.mBitangents[a].x);System.out.println(pMesh.mBitangents[a] .opMultiply( -1.0F));
		            pMesh.mBitangents[a]=new aiVector3D(pMesh.mBitangents[a].opMultiply(-1.0F));}
		    }
		}
		...
		public void ProcessMaterial( aiMaterial _mat)
		{
		    ...
		    for (int a = 0; a < mat.mNumProperties;++a)   {
		        ...
		        if (string.strcmp( prop.mKey.data, "$tex.mapaxis".getBytes(Charset.forName("UTF-8")))==0)    {
		            ...
		            pff.z = pff.z * -1.0F;
		            prop.mData=Bytes.serialize(pff);
		        }
		    }
		}
	...
	public static class FlipUVsProcess {
...
//             DefaultLogger::get()->debug("FlipWindingOrderProcess finished");
        ...
        public void ProcessMaterial (aiMaterial _mat)
        {
            ...
            for (int a = 0; a < mat.mNumProperties;++a)   {
                ...
                if (string.strcmp( prop.mKey, "$tex.uvtrafo")==0)    {
                    assert( prop.mDataLength >= (material.aiUVTransform));  /* something is wrong with the validation if we end up here */
                    material.aiUVTransform uv = Bytes.deserializeaiUVTransform(prop.mData);

                    // just flip it, that's everything
                    uv.mTranslation.y *= -1.0f;
                    uv.mRotation *= -1.0f;
                }
            }
        }
...
//                    DefaultLogger::get()->debug("FlipWindingOrderProcess finished");
               ...
               public void ProcessMesh(aiMesh pMesh) {
                   ...
                   for (int a = 0; a < pMesh.mNumFaces; a++) {
                       ...
                       for (int b = 0; b < face.mNumIndices / 2.0; b++) {
